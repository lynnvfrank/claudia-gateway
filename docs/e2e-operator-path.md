...
- **BiFrost** running and reachable at **`upstream.base_url`** (e.g. **`make claudia-serve`** or **`make up`** after **`make install`**), with provider keys in the environment for **`config/bifrost.config.json`**.
...
2. **`make install`** (once) and **`make claudia-serve`** or **`make up`**.
