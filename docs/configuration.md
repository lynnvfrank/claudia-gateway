...
- **Upstream:** **`litellm.base_url`**, **`litellm.api_key_env`**, **`health.*`**, **`routing.fallback_chain`**, **`paths.*`** — see tables below. (YAML still uses the **`litellm.*`** prefix for historical reasons; the upstream is your OpenAI-compatible proxy, typically BiFrost.)
...
**`GET /health`** returns JSON including **`checks.litellm`** (upstream probe). **`GET /v1/models`** prepends the virtual **`Claudia-<semver>`** model, then merges BiFrost’s catalog when available. **`POST /v1/chat/completions`** validates the gateway Bearer token, applies routing for the virtual model, and walks the fallback chain on 429/selected 5xx.
...
|----------|----------|-------------|
| **`CLAUDIA_UPSTREAM_API_KEY`** | Yes for typical BiFrost setups | Bearer token the gateway sends to its OpenAI-compatible upstream (`litellm.base_url`). Name is configurable via **`litellm.api_key_env`** in **`gateway.yaml`** (default **`CLAUDIA_UPSTREAM_API_KEY`**). For BiFrost without governance virtual keys, any non-empty placeholder works. |
...
| **`litellm.base_url`** | OpenAI-compatible upstream root (no trailing slash required), e.g. **`http://127.0.0.1:8080`** for local BiFrost. |
